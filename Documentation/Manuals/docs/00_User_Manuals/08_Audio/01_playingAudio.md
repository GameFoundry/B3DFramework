...
# Volume
Volume of the audio source can be controlled through @b3d::CAudioSource::setVolume. The provided value ranges from 0 to 1.
...
# Pitch
Pitch can be controlled through @b3d::CAudioSource::setPitch by providing a pitch multiplier. Values larger than 1 yield a higher pitch, while values smaller than 1 yield a lower pitch.
...
# Seeking
You can seek to a specific position within the currently assigned audio clip by calling @b3d::CAudioSource::setTime. It accepts a time in seconds. If clip is currently playing, the playback will skip to the provided time. If clip is currently paused, the clip will resume from the provided time the next time **CAudioSource::play()** is called.
...
# Loop
By default when the playback reaches the end of the current audio clip, the playback will end. You can ensure the playback loops instead by calling @b3d::CAudioSource::setIsLooping.
...
## Listener
Listener provides a reference point used for 3D sound effects. It is represented with a @b3d::CAudioListener component. It requires no additional properties aside from being present in the scene.
...
## Attenuation
Attenuation determines how quickly does audio volume drop off as the listener moves further from the audio source. Change it through @b3d::CAudioSource::setAttenuation. Higher values means the sound attenuates more quickly with distance.

You can further control attenuation by setting @b3d::CAudioSource::setMinDistance. If the listener is closer to the source than minimum distance, then the audio will play at full volume. This ensures that the effect of attenuation can be avoided when the listener is close to the source.
...
# Global controls
Audio options can also be controlled globally though the @b3d::Audio system, accessible though @b3d::GetAudio. 

You can change the audio volume globally by calling @b3d::Audio::setVolume.
~~~~~~~~~~~~~{.cpp}
// Mute all sounds
GetAudio().setVolume(0.0f);
~~~~~~~~~~~~~

You can also pause/unpause all sounds globally by calling @b3d::Audio::setPaused.
~~~~~~~~~~~~~{.cpp}
// Pause all sounds
...
Sometimes you do not need all the features offered by **AudioSource** component, and just want to play a sound with no extra options. In that case you can call @b3d::Audio::play and provide it with an **AudioClip** to play. The clip will play at the provided volume & position. It will stop once it ends (no looping), and you won't have any control over its playback once it starts. This is useful for short one-shot effects.
...
To retrieve the default audio device (the one user selected in the operating system), call @b3d::Audio::getDefaultDevice.
