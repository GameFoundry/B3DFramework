...
All available containers:
 ...
 - @bs::TInlineArray - A dynamically sized container that statically allocates enough room for N elements. If the element count exceeds the statically allocated buffer size the vector falls back to general purpose dynamic allocator.
 - @bs::TArray - Dynamically sized array, similar to **Vector** but without relying on the standard library.
