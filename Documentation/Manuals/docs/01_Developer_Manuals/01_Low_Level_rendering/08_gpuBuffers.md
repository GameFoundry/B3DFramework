...
## D3D12 plugin layout
The D3D12 plugin uses two allocator levels. **D3D12HeapBackend** creates native `ID3D12Heap` objects used for placed resources. **D3D12BufferPool** then creates large buffer resources on those heaps and suballocates their byte ranges through one @b3d::TGpuTlsfAllocator per compatible heap/resource-flag class (default, UAV-capable default, upload, and readback). Small logical buffers therefore share an `ID3D12Resource`; very large allocations naturally receive a dedicated page. Upload and readback pages are persistently mapped once, while each logical buffer exposes its mapped pointer, GPU virtual address, and descriptors at the slice offset.

Logical hazards and image layouts remain authoritative in the core resource tracker. The D3D12 barrier helper maps those scopes to enhanced buffer and texture barriers; it does not maintain a duplicate legacy resource-state map. A native buffer page is itself tracked as a buffer resource, receiving writes from every logical slice for write-after-write synchronization within a recording scope and across queues. Reads remain associated only with their logical buffers, so non-overlapping page reads and writes are not unnecessarily serialized. The helper aliases logical barriers onto the page's native barrier chain because both resource identities reference the same `ID3D12Resource`.
...
## Defragmentation
@b3d::TGpuTlsfAllocator::Defrag compacts live allocations to free up empty heaps and reduce fragmentation. The pass walks heaps high-index → low-index and within each heap walks the per-heap physical chain offset-high → offset-low. Eligible allocations are moved into either a lower-offset slot in the same heap (within-heap compaction) or a slot in a lower-index heap (multi-heap drain). Source-slot release follows the configured @b3d::GpuAllocatorFreeDeferralMode (see above): under @b3d::GpuAllocatorFreeDeferralMode::FrameTracker the allocator retires the source against the current frame index and drains it from the deferred-free queue once the frame is complete; under @b3d::GpuAllocatorFreeDeferralMode::ResourceLifecycle the allocator does not track the source — the consumer's destructor frees it through the allocation's carried allocator.
