...
To create a **Flags<Enum, Storage>** type for an enum simply define a `typedef` with your enum type provided as the template parameter. You must also follow that definition with a @B3D_FLAGS_OPERATORS macro in order to ensure all operators are properly defined.
~~~~~~~~~~~~~{.cpp}
