...
namespace b3d
{
	...
	namespace render
	{
		...
		/** Additional behavior requested when issuing an image barrier. */
		enum class GpuImageBarrierFlag : u8
		{
			None = 0,
			DiscardContents = 1 << 0 /**< Existing contents need not be preserved by the barrier. */
		};

		typedef Flags<GpuImageBarrierFlag, u8> GpuImageBarrierFlags;
		B3D_FLAGS_OPERATORS_EXT(GpuImageBarrierFlag, u8)
