...
namespace bs
{
	...
	namespace ct
	{
		LinuxRenderWindow::LinuxRenderWindow(const RENDER_WINDOW_DESC& desc, u32 windowId, VulkanRenderAPI& renderAPI)
			: RenderWindow(desc, windowId), mRenderAPI(renderAPI), mRequiresNewBackBuffer(true), mWindow(nullptr), mProperties(desc), mSyncedProperties(desc), mIsChild(false), mShowOnSwap(false)
		{}
