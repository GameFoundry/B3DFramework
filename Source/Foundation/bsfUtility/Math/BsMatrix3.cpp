...
namespace bs
{
	...
	void Matrix3::GolubKahanStep (Matrix3& matA, Matrix3& matL, Matrix3& matR)
	{
		...
		float z = matA[0][1];
		float invLength = Math::InverseSqrt(y * y + z * z);
		...
		y = matA[0][0];
		z = matA[1][0];
		invLength = Math::InverseSqrt(y * y + z * z);
		sin = z*invLength;
		cos = -y*invLength;
		...
		for (unsigned int i = 0; i < SVD_MAX_ITERS; i++)
		{
			...
			if (test1)
			{
				...
				else
				{
					...
					cos0 = Math::InverseSqrt(1.0f + tan0 * tan0);
					sin0 = tan0*cos0;
					...
					cos1 = Math::InverseSqrt(1.0f + tan1 * tan1);
					sin1 = -tan1*cos1;
	...
	{
		// Compute q0
		float invLength = Math::InverseSqrt(m[0][0] * m[0][0] + m[1][0] * m[1][0] + m[2][0] * m[2][0]);
		...
		invLength = Math::InverseSqrt(m[0][1] * m[0][1] + m[1][1] * m[1][1] + m[2][1] * m[2][1]);
		...
		invLength = Math::InverseSqrt(m[0][2] * m[0][2] + m[1][2] * m[1][2] + m[2][2] * m[2][2]);
	...
	void Matrix3::QDUDecomposition(Matrix3& matQ, Vector3& vecD, Vector3& vecU) const
	{
		...
		invLength = Math::InverseSqrt(matQ[0][1] * matQ[0][1] + matQ[1][1] * matQ[1][1] + matQ[2][1] * matQ[2][1]);
		matQ[0][1] *= invLength;
		matQ[1][1] *= invLength;
		matQ[2][1] *= invLength;
		...
		invLength = Math::InverseSqrt(matQ[0][2] * matQ[0][2] + matQ[1][2] * matQ[1][2] + matQ[2][2] * matQ[2][2]);
		matQ[0][2] *= invLength;
		matQ[1][2] *= invLength;
		matQ[2][2] *= invLength;
